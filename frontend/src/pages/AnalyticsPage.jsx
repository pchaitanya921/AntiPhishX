...
const SAMPLE_DATA = {
    ...
    topicsByCategory: [
        { _id: 'executive_intelligence', count: 75 },
        { _id: 'tactical_defense', count: 75 },
        { _id: 'cognitive_security', count: 75 },
        { _id: 'advanced_ai_adaptive', count: 75 },
    ],
};
...
export default function AnalyticsPage() {
    ...
    useEffect(() => {
        if (user?.role === 'admin') {
            adminAPI.getPlatformAnalytics().then(res => {
                if (res.data) setData(res.data.data || SAMPLE_DATA);
            }).catch(console.error);
        } else {
            analyticsAPI.getUserAnalytics().then(res => {
                if (res.data) setData(res.data.data || SAMPLE_DATA);
            }).catch(console.error);
        }
    }, [user]);
    ...
    return (
        <div className="space-y-10">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-4 border-b border-white/5">
                <div>
                    <h1 className="text-5xl font-black mb-3 tracking-tighter italic">
                        {user?.role === 'admin' ? 'Platform' : 'Personal'} <span className="cyber-gradient-text">Analytics</span>
                    </h1>
                    <p className="text-white/40 font-bold uppercase tracking-widest text-xs">
                        {user?.role === 'admin' ? 'Real-time system telemetry & learner metrics' : 'Your personal growth & performance metrics'}
                    </p>
                </div>
            </div>
            ...
            {user?.role === 'admin' ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <StatCard
                        ...
                        value={`${Math.round(data?.quizPassRate || 0)}%`}
                    ...
                    <StatCard
                        icon={FlaskConical}
                        label="Lab Attempts"
                        value={data?.labAttempts || 0}
                        subValue="Total simulations run"
                        color="cyan"
                    />
                    <StatCard
                        icon={TrendingUp}
            ...
            <PermissionWrapper permission={PERMISSIONS.VIEW_GLOBAL_ANALYTICS} hideInsteadOfDisable={true}>
